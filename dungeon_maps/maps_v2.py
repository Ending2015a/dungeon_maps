...
def _masked_gather(
  ...
  Args:
      ...
      fill_value (float, optional): default values to fill in. Defaults to INVALID.
...
def orth_project(
  ...
  Args:
      ...
      valid_map (torch.Tensor, optional): mask to indicate which pixels are valid or
          invalid. Invalid pixels are discard during scattering.
  ...
  Returns:
      ...
      torch.Tensor, optional: height maps, indicateing the maximum height of each
          ...
          are set to -inf a.k.a. `INVALID` for invalid regions. One can use `-np.inf`,
          `INVALID` or use `masks` to test if the pixels are valid or not.
  """
  if _validate_args:
    ...
    if valid_map is not None:
      device = depth_map.device
      valid_map = utils.validate_tensors(
        valid_map, same_device=device
      )
  ...
  if get_height_map:
    ...
    if indices is not None:
      # Note that only Reduction.max, Reduction.min has indices
      # being returned, for other types of reduction methods
      # we need to reproject the depth map seperately with
      # Reduction.max. The default fill_value is set to INVALID!
...
def depth_map_to_point_cloud(
  ...
  if _validate_args:
    ...
    if valid_map is not None:
      device = depth_map.device
      valid_map = utils.validate_tensors(valid_map, same_device=device)
      valid_map = utils.to_4D_image(valid_map) # (b, c, h, w)
  device = depth_map.device
...
def project(
  ...
  Returns:
      ...
      torch.Tensor: masks in shape (b, ..., mh, mw), False if the value is invalid,
          True otherwise.
      torch.Tensor, None: if `reduction` is set to `Reduction.min` or `Reduction.max`,
          this is the scattered indices (`coords`' indices) in shape (b, ..., mh, mw)
          where -1 if there is no value scattered at that pixels. For other reduction
          methods, this is None.
  """
  if _validate_args:
  ...
  maps, scattered_indices = utils.scatter_tensor(
    canvas = canvas,
    indices = indices,
    values = values,
    masks = valid_area,
  ...
  #TODO: if scaattered_indices is None, how do we get the masks?
  ...
  def __init__(
    ...
    Args:
        ...
        trunc_depth_max (float, optional): maximum depth values to truncate (unit).
            Defaults to None.
        trunc_height_max (float, optional): TODO
        ...
        fill_value (float, optional): default values to fill in the map when
            projecting. Defaults to INVALID.
        reduction (Reduction, optinoal): TODO
