...
_ScatterMethods: Dict[Reduction, Callable] = {
  ...
  Reduction.prod: torch_scatter.scatter_prod
}
...
def scatter_tensor(
  ...
  Note that the second returned tensor has two types:
  1. If the reduction method is set to `Reduction.max` or `Reduction.min`, the
      second tensor is the indices or scattered points (like argmax or argmin, see
      torch_scatter.scatter_max), where -1 for invalid scattering. torch.int64
  2. For other reduction methods, the second tensor is a masks indicates which
      pixels are scattered or not. True for valid (or scattered) pixels False for
      otherwise. torch.bool
  ...
  Returns:
      ...
      torch.Tensor: indices or masks. See above.
  """
  if _validate_args:
  ...
  if reduction in [Reduction.min, Reduction.max]:
    if fill_value is not None:
      flat_canvas.fill_(fill_value)
    _, flat_indices = scatter_method(
    ...
    # Here we create an empty canvas container for scattering because we
    # want to know which pixels are scattered and which are not. We first
    # fill in -np.inf a.k.a INVALID to the empty canvas, and perform
    # scattering. We then fill in the invalid regions (non-scattered)
    # with the original values on original canvas
    # Initialize empty canvas with INVALID
    flat_canvas_cont = torch.full_like(flat_canvas, INVALID)
    _ = scatter_method(
      flat_values, flat_indices, dim=-1, out=flat_canvas_cont
    )
    # slice off dummy channel
    flat_canvas_cont = flat_canvas_cont[..., 1:] # (b..., d1*...*dn)
    canvas_cont = flat_canvas_cont.view(canvas.shape) # (b..., dn, ..., d1)
    # Get mask from scattered canvas container
    masks = ~(canvas_cont == INVALID)
    canvas = torch.where(masks, canvas_cont, canvas)
    return canvas, masks
